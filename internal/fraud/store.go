package fraud

import (
	"encoding/binary"
	"errors"
	"fmt"
	"os"
	"unsafe"
)

const storeMagic = "RNF1"

// Store holds loaded reference vectors and labels (refs.bin).
type Store struct {
	raw    []byte
	n      int
	dim    int
	points []float32 // len n*dim, view into raw
	labels []byte    // len n
}

func (s *Store) N() int   { return s.n }
func (s *Store) Dim() int { return s.dim }

// Points returns the row-major float32 matrix (length n*dim).
func (s *Store) Points() []float32 { return s.points }

// LoadStore reads the binary format produced by cmd/prepare.
func LoadStore(path string) (*Store, error) {
	raw, err := os.ReadFile(path)
	if err != nil {
		return nil, err
	}
	if len(raw) < 16 {
		return nil, errors.New("refs file too small")
	}
	if string(raw[0:4]) != storeMagic {
		return nil, fmt.Errorf("bad magic %q", raw[0:4])
	}
	ver := binary.LittleEndian.Uint32(raw[4:8])
	if ver != 1 {
		return nil, fmt.Errorf("unsupported version %d", ver)
	}
	n := int(binary.LittleEndian.Uint32(raw[8:12]))
	dim := int(binary.LittleEndian.Uint32(raw[12:16]))
	if n <= 0 || dim != 14 {
		return nil, fmt.Errorf("invalid header n=%d dim=%d", n, dim)
	}
	vecBytes := n * dim * 4
	offVec := 16
	offLab := offVec + vecBytes
	if len(raw) < offLab+n {
		return nil, fmt.Errorf("truncated file: need %d bytes", offLab+n)
	}
	vecRegion := raw[offVec:offLab]
	points := bytesToFloat32Slice(vecRegion)
	labels := raw[offLab : offLab+n]
	return &Store{raw: raw, n: n, dim: dim, points: points, labels: labels}, nil
}

func bytesToFloat32Slice(b []byte) []float32 {
	if len(b) == 0 {
		return nil
	}
	if len(b)%4 != 0 {
		panic("unaligned float32 buffer")
	}
	return unsafe.Slice((*float32)(unsafe.Pointer(&b[0])), len(b)/4)
}
